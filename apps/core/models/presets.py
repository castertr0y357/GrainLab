...
class DoughCategory(models.Model):
    ...
    base_yeast = models.FloatField(
        default=0.015, help_text="Yeast ratio relative to flour (if chemically/yeast leavened)"
    )
...
class FormFactor(models.Model):
    ...
    name = models.CharField(max_length=100)
    slug = models.SlugField(max_length=100, unique=True, db_index=True)
    is_portioned = models.BooleanField(
        default=False, help_text="True if baked in count-based units rather than a single loaf"
    )
    ...
    is_enriched_profile = models.BooleanField(
        default=False, help_text="Determines if it uses enriched target probe temp (190F) vs lean (205F)"
    )
...
class BreadPreset(models.Model):
    ...
    dough_category = models.ForeignKey(DoughCategory, on_delete=models.CASCADE, db_index=True, related_name="presets")
    form_factor = models.ForeignKey(FormFactor, on_delete=models.CASCADE, db_index=True, related_name="presets")
    ...
    crumb_preview = models.CharField(
        max_length=50, default="Balanced", help_text="Crumb Structure Preview (Open, Balanced, or Even)"
    )
