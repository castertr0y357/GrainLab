    ...
    def get(self, request, category, archetype):
        ...
        try:
            ...
            if recipe.get("wheat_berry_mix"):
                ...
                if gt == "whole_wheat":
                    ingredients_list.append("Whole Wheat Flour")
                elif gt == "spelt":
                    ingredients_list.append("Spelt Flour")
                elif gt == "kamut":
                    ingredients_list.append("Kamut Flour")
                elif gt == "einkorn":
                    ingredients_list.append("Einkorn Flour")
                else:
                    ingredients_list.append("Flour Base")

            for item in recipe.get("liquid_items", []):
                ingredients_list.append(item.get("name", "Liquid"))
            if not recipe.get("liquid_items") and recipe.get("added_water", 0) > 0:
                ingredients_list.append("Water")
            for item in recipe.get("fat_items", []):
                ingredients_list.append(item.get("name", "Fat"))
            for item in recipe.get("sugar_items", []):
                ingredients_list.append(item.get("name", "Sugar"))
            for item in recipe.get("binder_items", []):
                ingredients_list.append(item.get("name", "Binder"))

            if recipe.get("salt_item"):
                ingredients_list.append(recipe["salt_item"].get("name", "Salt"))
            elif recipe.get("salt", 0) > 0:
                ingredients_list.append("Fine Sea Salt")

            if recipe.get("commercial_yeast_item"):
                ingredients_list.append(recipe["commercial_yeast_item"].get("name", "Yeast"))
            if recipe.get("starter_levain_item"):
                ingredients_list.append(recipe["starter_levain_item"].get("name", "Sourdough Starter"))
            ...
            # Build flat ingredient weights mapping
            ingredient_weights = {}
            ...
            if not recipe.get("lipid_items") and not recipe.get("fat_items") and recipe.get("fat_weight", 0) > 0:
                ingredient_weights[recipe.get("fat_substitute_label", "Unsalted Butter")] = float(
                    recipe.get("fat_weight", 0)
                )
            ...
            if (
                not recipe.get("sweetener_items")
                and not recipe.get("sugar_items")
                and recipe.get("sugar_weight", 0) > 0
            ):
            ...
            if recipe.get("salt_item"):
                ingredient_weights[recipe["salt_item"].get("name", "Salt")] = float(
                    recipe["salt_item"].get("weight", 0)
                )
            elif recipe.get("salt", 0) > 0:
...
class FinalRecipeAIView(View):
    def get(self, request, category, archetype):
        ...
        state = get_calculator_state(request)
        if not state:
            state = {"selected_master": category, "preset_slug": archetype, "global_ai_enabled": True}
