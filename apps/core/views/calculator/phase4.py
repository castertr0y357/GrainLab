...
from apps.core.models import DoughCategory, WheatBerry
...
class Phase4View(View):
    def get(self, request, category, archetype):
        ...
        if state.get("selected_master") != category or state.get("preset_slug") != archetype:
            update_calculator_state(
                request, {"selected_master": category, "preset_slug": archetype, "current_phase": 4}
            )
            state = get_calculator_state(request)
        ...
        if isinstance(flav_inc, str):
            try:
                flav_inc = json.loads(flav_inc)
            except:
                flav_inc = []

        context = {
            "berries": WheatBerry.objects.filter(
                is_active=True
            ),  # In case we need it, but the state has active_berries
