...
def calculate_local_compatibility_from_specs(wb, engine, preset_slug=None, active_archetype_id=None) -> dict:
    ...
    if elasticity == "minimal_to_none" and binding in ["negligible", "moderate"]:
        ...
        elasticity == "extreme_tensile"
        and binding == "negligible"
        or elasticity == "minimal_to_none"
        and binding == "extreme"
    ):
        tier = "not-recommended"
    ...
    return {"tier": tier, "reasoning": justification}
...
def optimize_grain_blend(preset_slug: str, preset_name: str, active_berries: list) -> tuple[dict, str | None] | None:
    ...
    user_prompt = json.dumps({"preset_slug": preset_slug, "preset_name": preset_name, "active_berries": berries_data})
        ...
        if res and "evaluation_result" in res:
            ...
            res_dict = {
                ...
                "reasoning": eval_result.get(
                    "technical_justification", "Analyzed physical targets and chemical profile successfully."
                ),
            }
            cache.set(cache_key, res_dict, timeout=None)
            return res_dict
...
def get_fallback_variants(
    engine_id: str, active_archetype_id: str, creativity_level: int, pref_slugs: list, exclude_names: list = None
) -> dict:
    cat_slug = engine_id
...
def stream_creativity_recipes(
    engine_id: str, active_archetype_id: str, inventory: list, level: str = "", active_variation_id: str = None
):
    """
...
def stream_creativity_variants(
    engine_id: str,
...
def generate_recipe_details(
    ...
    try:
        ...
        result = call_gemma_api(
            system_prompt, user_prompt, expected_keys=["secondary_ingredients", "recommended_grain_ids"]
        )
...
def stream_recipe_details(
    ...
    return get_local_recipe_details(
        recipe_slug, engine_id, active_archetype_id, selected_grains, recipe_name=recipe_name
    )


def get_local_recipe_details(
    recipe_slug: str,
    engine_id: str,
    active_archetype_id: str,
    selected_grains: str,
...
def stream_generate_substitutes(
    engine_id: str,
    active_archetype_id: str,
    recipe_slug: str,
    recipe_name: str,
...
def generate_recipe_percentages(
    engine_id: str,
    active_archetype_id: str,
    recipe_slug: str,
    recipe_name: str,
