...
class DynamicRouteScannerTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.category = DoughCategory.objects.create(
            name="Lean & Crusty", slug="lean-crusty", base_hydration=0.68, base_fat=0.0, base_sugar=0.0
        )
        cls.form_factor = FormFactor.objects.create(name="Standard 9x5 Loaf Pan", slug="loaf-pan", target_weight=900.0)
        cls.preset = BreadPreset.objects.create(
            name="Bagel", slug="bagel", dough_category=cls.category, form_factor=cls.form_factor
        )
    ...
    def test_route_scanner(self):
        ...
        for route_str, name in all_routes:
            ...
            if name == "load_preset":
                ...
                wb, _ = WheatBerry.all_objects.get_or_create(
                    name="Temp Route Scan Berry", defaults={"protein_content": 12.0}
                )
                if wb.deleted_at:
                    wb.deleted_at = None
                    wb.save()
                ...
                eq, _ = Equipment.all_objects.get_or_create(
                    name="Temp Route Scan Eq", defaults={"equipment_type": "mixer"}
                )
                if eq.deleted_at:
                    eq.deleted_at = None
                    eq.save()
                ...
                wb, _ = WheatBerry.all_objects.get_or_create(
                    name="Temp Route Scan Redo Berry", defaults={"protein_content": 12.0}
                )
                if wb.deleted_at:
                    wb.deleted_at = None
                    wb.save()
            ...
            if name == "generate_variants" or name == "generate_creativity_recipes":
                response = client.get(url + "?engine_id=lean-crusty&active_archetype_id=classic_sourdough")
            elif name == "ai_recipe_details":
                response = client.get(
                    url
                    + "?recipe_slug=classic_sourdough_level1_1&engine_id=lean-crusty&active_archetype_id=classic_sourdough"
                )
            else:
...
class InventoryAndEquipmentTests(TestCase):
    ...
    def test_views_selected_grains_integration(self):
        ...
        category = DoughCategory.objects.create(
            name="Lean & Crusty", slug="lean-crusty-test", base_hydration=0.68, base_fat=0.0, base_sugar=0.0
        )
        form_factor = FormFactor.objects.create(name="Standard 9x5 Loaf Pan", slug="loaf-pan-test", target_weight=900.0)
        wb1 = WheatBerry.objects.create(
            name="Grain A", protein_content=14.0, hardness="hard", moisture_absorption_coef=1.0
        )
        wb2 = WheatBerry.objects.create(
            name="Grain B", protein_content=10.0, hardness="soft", moisture_absorption_coef=0.95
        )
